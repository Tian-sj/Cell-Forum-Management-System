...
<html>
	...
	<body style="background-color: #f1f1f1">
		<div id="ct"></div>
		<form action="OwnersMainSS.bin" method="post">
		...
		<div id="main">
			...
			<ul id="menu" style="top: 75px">
				...
				<li><a href="With_audit_message.jsp?username=<%=request.getParameter("username")%>">待审核的帖子</a></li>
				<li style="float: right"><a class="active" href="Forum_login.jsp">业主:<%=request.getParameter("username")%>&nbsp;退出登录</a></li>
			</ul>
			...
			<table width="100%">
				<tr>
					...
					<td width="70%"><a href="PostsShow_O.jsp?id=<%=rs.getString(6) %>&username=<%=request.getParameter("username")%>&ss=<%=ss %>"><%=rs.getString(2) %></a></td>
			...
			<div style="background-image:url('images/form.jpg');">
				<form action="Published.bin" method="post" onsubmit="return Check();">
					<input type="hidden" name = "username" value=<%=request.getParameter("username") %>>
